...
var db;
MongoClient.connect('mongodb://185.169.55.163:27017/ds', { useNewUrlParser: true, useUnifiedTopology: true }, function (err, client) {
    if (err) throw err
...
exports.deleteFind = (query, whichCollection) => {
    return db.collection(whichCollection).deleteOne(query)
}
...
exports.update = (query, data, whichCollection) => {
    return db.collection(whichCollection).updateOne(query, { $set: data, $currentDate: { lastModified: true } })
}
...
exports.user = (mail) => {
    return db.collection("users").find({ mail: mail }).toArray().then(user => {
        return user[0]
    })
}
// setTimeout(() => {
