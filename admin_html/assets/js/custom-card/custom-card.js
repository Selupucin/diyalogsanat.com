var customcard = {
    init: function () {
        $(".card-header-right .close-card").on('click', function () {
            ...
            var card = $(port).children('.card-footer').slideToggle();
            $(this).toggleClass("icofont-minus").fadeIn('slow');
            $(this).toggleClass("icofont-plus").fadeIn('slow');
            if ($this.hasClass('icofont-plus')) {
                $(".card-header").css("border-bottom-width", "0")
                $(".card-header").css("transition-duration","0.4s")
            } else {
                $(".card-header").css({"border-bottom": "1px solid #374558","border-bottom-width": "1px"})
            }
